...
To illustrate how some of the other operations work on this sample data:
...
The same xref:#failure-handling-during-persistence[Failure handling during persistence] error handling detailed above can be used with sub-document operations, so the failures of individual sub-document KV calls do not have to fail the entire Spark job.
