...
Also see the examples under xref:https://github.com/couchbase/couchbase-spark-connector/tree/master/src/test/pyspark/examples[Couchbase Spark Connector PySpark examples], which include a guide to using the Couchbase Spark Connector together with Spark MLLib.
...
Also see the notebook examples under 
link:++https://github.com/couchbase/couchbase-spark-connector/blob/master/src/test/pyspark/examples/++[Couchbase Spark Connector PySpark examples].
...
.Operations supported in PySpark
[cols="1,1,1,1"]
|===
| Operation | Examples | PySpark Support | Scala & Java Support

| xref:spark-sql.adoc[DataFrame reads]
| * `spark.read.format("couchbase.query").load()`
* `spark.read.format("couchbase.analytics").load()`
* `spark.read.format("couchbase.enterprise-analytics").load()`
* `spark.read.format("couchbase.columnar").load()`
| ✅
| ✅

| xref:spark-sql.adoc#_dataframe_persistence[DataFrame writes]
| * `airlines.write.format("couchbase.kv").save()`
* `airlines.write.format("couchbase.query").save()`
| ✅
| ✅

| xref:spark-sql.adoc#_datasets[Datasets]
| `spark.read.format("couchbase.query").load().as[Airline]`
| ❌ (1)
| ✅

| xref:working-with-rdds.adoc[RDD operations]
| * `spark.sparkContext.couchbaseGet(Seq(Get("airline_10"))).collect()`
* `spark.sparkContext.couchbaseQuery[JsonObject](...).collect()`
* `spark.sparkContext...couchbaseUpsert(...).collect()`
| ❌ (2)
| ✅
|===
...
(2) RDD operations are not supported, as these require Scala specifics that are not supportable through the PySpark interface.
These operations include reading from KV and executing arbitrary SQL++, both of which use RDDs.
